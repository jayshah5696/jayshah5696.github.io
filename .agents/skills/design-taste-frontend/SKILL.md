...
**Cards, micro-specs & list interaction**
* **NO faux-telemetry spec tables or badge clutter on cards.** (`QUERY ENGINE: DuckDB`, `ORCHESTRATION: LangGraph`). Do not manufacture fake system specs or subtitle grids to fill card whitespace. Use clean typography, editorial line-height, and concise micro-pills.
* **NO floaty translations or scaling on dense lists.** `hover:-translate-y-*` or `scale-125` across repeated cards or compact rows creates vertical jitter and cognitive fatigue. Ground cards with clean `border-color` shifts and subtle shadow tints.
* **NO entrance delay (`--index`) leaking into `:hover` transitions (Emil Kowalski Rule).** Never leave `transition-delay` active on interactive elements. Fast cursor sweeps across adjacent cards must respond instantly (`transition-delay: 0s !important`) without lagging or ghost-rendering.
* **NO component isolation (The One-Off Trap).** Never redesign a timeline, card, or divider on one page without auditing sibling pages (`/about`, `/projects`, `/reads`, `/archives`, `/links`). Foundational motifs must have cross-surface parity.
...
- [ ] **Instant Hover / Zero-Delay Check (Emil Kowalski Rule)**: All high-frequency interactive elements (`a`, `button`, cards, list rows, tags) have `transition-delay: 0s !important`. No entrance delay (`--index`) leaks into `:hover`. Rapid cursor sweeps across lists trigger instantaneous feedback with zero phantom/delayed glitching.
- [ ] **Surface Parity / Anti-Isolation Check**: Foundation motifs (cards, timelines, dividers, tags) are not created as one-off islands. Sibling surfaces checked and harmonized.
- [ ] **No Faux-Telemetry Slop**: Cards do not contain synthetic key-value spec tables (`QUERY ENGINE: DuckDB`, `ORCHESTRATION: LangGraph`). Whitespace preserved; concise micro-pills only.
- [ ] **No Floaty Hover Physics on Dense Lists**: Banned `hover:-translate-y-*` or `scale-125` on dense card or timeline stacks. Use grounded border and background tint shifts.
